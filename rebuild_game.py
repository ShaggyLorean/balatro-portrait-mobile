...
def check_incremental(src_dir, output_file):
    cache = _load_cache()
    current_files = {}
    for root, dirs, files in os.walk(src_dir):
        for file in files:
            ...
            try:
                current_files[fp] = _file_hash(fp)
            except Exception:
                current_files[fp] = str(os.path.getmtime(fp))

    cached_files = cache.get('files', {})
    if os.path.exists(output_file) and current_files == cached_files:
        print("No changes detected since last build. Skipping.")
        print(f"  {output_file} is up to date.")
        return False
    _save_cache({'files': current_files})
    return True
...
def apply_readabletro_patch(src_dir, apply=True):
    ...
    if apply:
        ...
        for shader in shaders:
            ...
            s_bak = s_dst + ".bak"
            os.makedirs(os.path.dirname(s_dst), exist_ok=True)
...
def rebuild_game_love(apply_crt_patch_flag=False, apply_readabletro_flag=False):
    ...
    if not check_incremental(src_dir, output_file):
        if apply_crt_patch_flag:
