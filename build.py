...
def main():
    ...
    needs_setup = not os.path.exists("src") or not os.path.exists(os.path.join("src", "resources"))
    if needs_setup:
        print("Resources not found. We need to extract them from your Balatro installation first.")
        balatro_path = input("Please enter the path to Balatro.exe (e.g. C:\\Program Files (x86)\\Steam\\steamapps\\common\\Balatro\\Balatro.exe): ").strip().strip('"').strip("'")
        
        setup_args = [sys.executable, "setup.py", balatro_path]
        res = subprocess.run(setup_args)
        if res.returncode != 0:
            print("\nERROR: setup.py failed to extract the game.")
            sys.exit(1)
